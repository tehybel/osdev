...
- drawing pixels
	- linear frame buffer needed
	- needed to switch back to 16-bit real mode to query the BIOS
	- had to write assembly code for this

- graphics stack design
	- overview of how it all works together, what all the pieces do
	- user-mode graphics program
		- has a canvas (works via IPC)
		- spawned by display server

- display server 
	- writes to the LFB
		- it's mapped in by the kernel
	- z-buffer
	- manages windows
	- data structures etc. (Could be improved.)
	- had to be hand-optimized 

- user-mode graphics library
	- can render fonts
		- uses one linux font, explain how it works
	- can draw straight lines, rectangles
	- can draw windows
	- read source, what else can it do?
