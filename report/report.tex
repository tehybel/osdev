...
% The machine got stuck at some point during execution of the boot loader. When
% using QEMU we would debug this either by attaching a proper debugger or by
% inserting print statements through a serial connection into the code.
% Attaching a debugger to our netbook was infeasible, and we did not have a
% serial connection. We therefore needed another option for output. 
% 
% Before a graphical video mode has been set, the machine is in text mode; it
% displays text on the screen. In this mode a buffer representing the text on
% the screen resides at physical address \texttt{0xb8000}. We wrote to this
% buffer to display output for debugging. 


% TODO: <-------- I got to here.
% We should probably do away with the "booting from a hard drive" section,
% because it's not very interesting. 
% Just say that the boot loader got stuck since it didn't have USB support. We
% could then boot from hard drive instead, but that's too slow and tedious. So
% we adopted GRUB and that works well..
% 
% ^^^^^^^^^^^^^^^^
