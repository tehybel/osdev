...
% TODO: some references are missing, look at the output of pdflatex.
...
``labs", with each lab encapsulating a subset of kernel functionality. For
example, lab 4 involves multitasking and concurrency, while lab 5 comprises
the implementation of a custom file system. 

We wrote the kernel for the 32-bit x86 architecture. The kernel is written
in C, with some parts hand-written in assembly. We have used GCC
...
\caption{Interaction with the two graphical applications.}
\label{desktop}
\end{figure}
...
% TODO: this is a bit of a mess, make it clearer what we want to say.
...
a user space process. We have extended this infrastructure to support
concurrent processes and enabled their interaction through an IPC mechanism.
We also implemented a custom file system to let programs write persistent
changes to disk.

By designing and implementing a graphical user interface for our system we
have additionally seen how graphics work, from drawing a single pixel to the
end result with a display server managing windows and rendering fonts. 

We have seen how various features could be moved from the kernel to user space
according to exokernel principles. Some examples of this were the network
stack and the file system code. We have also gained experience writing drivers
for various devices, including a networking card and a PS/2 mouse.
...
% TODO: add more references (some links are in writeup txt files)
