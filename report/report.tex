...
user-mode process. The current lab concerned itself with the features needed
to run multiple processes concurrently, and having them interact. We
implemented preemptive multitasking, process forking, and inter-process
communication.


\subsection{Interacting with the LAPIC}
- motivation: the LAPIC is used to bring up more processors. It is also used
  to generate clock signals, which our scheduler needs.
- uses MMIO; hard part is getting the LAPIC mapped at a place so we can write
  to it
- TODO recall how this works

\subsection{Activating more processors}
- emulating multiple processors
- initially only the BP runs
- we need to boot up APs
- to do this, we have our LAPIC send an IPI to LAPICs of other processors
- this boots the AP in 16-bit real mode; must switch modes as described
  earlier.


\subsection{Ensuring mutual exclusion}
- can't have processors touching kernel structures concurrently
- big kernel lock
- means: we have true concurrency when running multiple user applications, but
  not in the kernel.
