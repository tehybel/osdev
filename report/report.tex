...
table of the process into \gls{cr3}.

We then issue the \texttt{iret} instruction, which
atomically sets the \gls{eip} (instruction pointer), \gls{esp} (stack
pointer), \gls{eflags}, and \gls{cs} registers. This transfers execution
to user-mode code. The lower two bits of the \gls{cs} register determine
the Current Privilege Level (\gls{cpl}) of the processor. This was previously
0, since the kernel runs in ring 0. By setting this to 3 during the
